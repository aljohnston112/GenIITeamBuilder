...
const std::vector<std::pair<std::string, int>> DamageCalculator::buffedAttackPowers = {
        {"Present",                120},
        {"Reversal",               0},
        {"Magnitude",              150},
        {"Hidden Power",           70},
...
const std::vector<std::pair<std::string, int>> DamageCalculator::nonBuffedAttackPowers = {
        {"Present",                0},
        {"Reversal",               0},
        {"Magnitude",              10},
        {"Hidden Power",           31},
...
double DamageCalculator::getAttackStat(const std::shared_ptr<PokemonState> &pokemonState, bool isSpecial) {
    return isSpecial ? pokemonState->specialAttack : pokemonState->attackStat;
}

double DamageCalculator::getDefenseStat(const std::shared_ptr<PokemonState> &pokemonState, bool isSpecial) {
    return isSpecial ? pokemonState->specialDefense : pokemonState->defenseStat;
}

int DamageCalculator::getAttackPower(
        const Attack &attack,
    ...
    if (attackName == "Low Kick") {
        movePower = AttackUtil::getLowKickPower(defender->pokemon.pokemonInformation.pounds);
    } else if (buff) {
...
std::pair<double, std::shared_ptr<Attack>> DamageCalculator::getMaxDamageAttackerCanDoToDefender(
    ...
    DefenseMultiplierMap defender_defense_multipliers = DefenseMultipliers::getDefenseMultipliersForTypes(
            defender_types
    );
    ...
    for (const auto &pair: attackers_attacks) {
        ...
        for (const auto &attack: attacks) {
            if (attacker_buffed || attack.accuracy == 100) {
                ...
                double defense_stat = getDefenseStat(defender_state, is_special);
